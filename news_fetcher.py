...
# === Tool Use 用のスキーマ定義 ===
_SELECT_TOOL = {
    "name": "select_top_news",
    "description": "選定したニュース候補を構造化して返す",
    "input_schema": {
        "type": "object",
        "properties": {
            "selected": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "theme_for_video": {"type": "string"},
                    },
                    "required": ["rank", "score", "title", "source", "url",
                                  "buzz_reason", "risk_check", "theme_for_video"],
                },
            },
        },
        "required": ["selected"],
...
_VERIFY_TOOL = {
    ...
    "description": "候補が過去投稿または手動ブロックリストと重複するかを返す",
    "input_schema": {
        "type": "object",
        "properties": {
...
def _call_claude_json(client, prompt, max_tokens=4000, label="claude", tool=None, max_retries=2):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            last_err = e
            print(f"  [warn][{label}] attempt {attempt}/{max_retries}: {e}", file=sys.stderr)
    raise last_err or RuntimeError(f"Claude tool_use failed after {max_retries} retries")
